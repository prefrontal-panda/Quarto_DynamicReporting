...
student_CG = pd.read_csv("PAT_ReportGen_Files/Student_CG.csv")
```
```{python}
...
# Loading year level dictionaries
with open("yrlvl_df.pkl", "rb") as f:
    year_level_dataframes_maths = pickle.load(f)
    year_level_dataframes_reading = pickle.load(f)
# Loading question substrand dictionaries
with open("q_strands.pkl", "rb") as f:
    year_level_q_strands_maths = pickle.load(f)
    year_level_q_strands_reading = pickle.load(f)

# Selecting classes
df_maths = class_maths.get(class_code, None)
df_reading = class_reading.get(class_code, None)

# Check if classes exist in the dictionary
has_maths = class_code in class_maths
has_reading = class_code in class_reading

# Check if there is any data for this class
if not has_maths and not has_reading:
    display(Markdown(f"## No Data Available\n\nNo PAT test data found for class {class_code}."))
    import sys
    sys.exit(0)

# Get the nested dictionaries (which contain year levels)
if has_maths:
    df_maths = class_maths[class_code]  # This is a dict like {'Year7': DataFrame, 'Year8': DataFrame}
else:
    df_maths = None
...
availability_msg = "## Data Availability\n\n"
if has_maths and has_reading:
    availability_msg += "PAT Maths and Reading data available for this class.\n"
elif has_maths:
    availability_msg += "PAT Maths data available.\n\nNote: PAT Reading data not available for this class.\n"
elif has_reading:
    availability_msg += "PAT Reading data available.\n\nNote: PAT Maths data not available for this class.\n"

display(Markdown(availability_msg))

# Get year levels from whichever dataset is available
if has_maths:
    year_levels_in_class = list(df_maths.keys())  # Get year levels from dictionary keys
elif has_reading:
    year_levels_in_class = list(df_reading.keys())
else:
    year_levels_in_class = []

# Convert to numeric for sorting (e.g., 'Year7' -> 7) - ensures year levels are printed in the correct order (e.g. 7, 8, 9, 10 instead of 10, 7, 8, 9)
year_levels_numeric = [int(yl.replace('Year', '')) for yl in year_levels_in_class]
```

```{python}
# Make a dictionary of substrands and ACER norms
substrand_norm_df = {
    'Year7': {
        'substrands_maths': maths_substrands_y7,
        'mean_maths': 131.6,
        'std_maths': 13.1,
        'mean_reading': 130.7,
        'std_reading': 13.0
    ...
    'Year8': {
        'substrands_maths': maths_substrands_y8,
        'mean_maths': 133.6,
        'std_maths': 12.3,
        'mean_reading': 132.6,
        'std_reading': 12.8
    ...
    'Year9': {
        'substrands_maths': maths_substrands_y9,
        'mean_maths': 135.4,
        'std_maths': 12.9,
        'mean_reading': 135.5,
        'std_reading': 12.4
    ...
    'Year10': {
        'substrands_maths': maths_substrands_y10,
        'mean_maths': 137.1,
        'std_maths': 12.4,
        'mean_reading': 140.5,
        'std_reading': 12.2
...
def compare_plot(dataframe, class_df, pct_corrct_df,
    ...
    percent_correct['Strand'] = pct_corrct_df['Strand']
...
# Get list of year levels and student counts
if has_maths:
    # Combine all year levels from the nested dict to count students
    all_year_levels = []
    for year_key, df in df_maths.items():
        all_year_levels.extend(df['StudentYearLevel'].tolist())
    stud_year_level = pd.Series(all_year_levels).value_counts().to_dict()
elif has_reading:
    # Combine all year levels from the nested dict
    all_year_levels = []
    for year_key, df in df_reading.items():
        all_year_levels.extend(df['StudentYearLevel'].tolist())
    stud_year_level = pd.Series(all_year_levels).value_counts().to_dict()
else:
    stud_year_level = {}
...
```{python}
# Calling plot
# loop through dataframe and get each year level
for year_level in sorted(year_levels_numeric):
    display(Markdown(f"## Year {year_level} Results"))

    # Get year-specific data
    # Key
    year_key = f'Year{year_level}' # Converting back to 'Year7', 'Year10', etc.
    # Dataframe
    year_data = substrand_norm_df[year_key]

    # Call relevant q_strands_match dataframe
    q_strands_maths = year_level_q_strands_maths[year_key]
    q_strands_reading = year_level_q_strands_reading[year_key]

    # Maths
    if has_maths and year_key in df_maths:
        df_maths_year = df_maths[year_key] # Get dataframe

        # Plot
        compare_plot(dataframe = year_data['substrands_maths'], class_df = df_maths_year,
                    pct_corrct_df = q_strands_maths,
                    strand_col_name = 'Strand', substrand_col_name = 'Sub-strand',
                    test = 'PAT Maths', plot_title = None)
    else:
        display(Markdown("*Note: PAT Maths data not available for this class.*\n"))
    
    # Reading
    if has_reading and year_key in df_reading:
        df_reading_year = df_reading[year_key]
        
        compare_plot(dataframe = q_strands_reading, class_df = df_reading_year,
                    pct_corrct_df= q_strands_reading,
                    strand_col_name = 'Strand', substrand_col_name = 'Strand',
                    test = 'PAT Reading', plot_title = None)
    else:
        display(Markdown("*Note: PAT Reading data not available for this class.*\n"))    
...
## Student Scale Score vs Year level-wide Scores
...
for year_level in sorted(year_levels_numeric):
    ...
    # Get year-level comparison data
    year_maths = year_level_dataframes_maths[year_key] # contains the students_math_match_yX
    year_reading = year_level_dataframes_reading[year_key]

    # Maths
    if has_maths and year_key in df_maths:
        df_maths_year = df_maths[year_key] # Access year-level dataframe from the nested dictionary

        # Plot
        student_vs_year_score(
            df1 = year_maths, df2 = df_maths_year,
    ...
    if has_reading and year_key in df_reading:
        ...
        # Plot
        student_vs_year_score(
            df1 = year_reading, df2 = df_reading_year,
            yearlevel_scores = 'Scale', student_scores = 'Scale',
            student_names = 'StudentNameExternal',
            title = 'PAT Maths - Student Scores vs School-wide Year-Level Scores'
        )
...
```{python}
# Get campus where class is taught
if has_maths:
    # Get any row from any year level to find the campus
    first_year_key = list(df_maths.keys())[0]
    campuses = df_maths[first_year_key]['Campus'].unique()
elif has_reading:
    first_year_key = list(df_reading.keys())[0]
    campuses = df_reading[first_year_key]['Campus'].unique()
else:
    campuses = []
...
for year_level in sorted(year_levels_numeric):
    ...
    # Get year-level comparison data
    students_maths_match = year_level_dataframes_maths[year_key]
    students_reading_match = year_level_dataframes_reading[year_key]
    # Get campus set
    ...
    # Maths
    if has_maths and year_key in df_maths:
        df_maths_year = df_maths[year_key]

        # Plot
        student_vs_year_score(
            df1 = campus_set_maths, df2 = df_maths_year,
            yearlevel_scores = 'Scale', student_scores = 'Scale',
            student_names = 'StudentNameExternal',
    ...
    if has_reading and year_key in df_reading:
        ...
        # Plot
        student_vs_year_score(
            df1 = campus_set_reading, df2 = df_reading_year,
            yearlevel_scores = 'Scale', student_scores = 'Scale',
            student_names = 'StudentNameExternal',
...
for year_level in sorted(year_levels_numeric):
    ...
    if has_maths and year_key in df_maths:
        ...
        # Plot
        score_bellcurve(
            dataframe = df_maths_year, mean = year_data['mean_maths'],
            std=year_data['std_maths'], student_scale_col='Scale',
    ...
    if has_reading and year_key in df_reading:
        ...
        # Plot
        score_bellcurve(
            dataframe = df_reading_year, mean = year_data['mean_reading'],
            std=year_data['std_reading'], student_scale_col='Scale',
...
for year_level in sorted(year_levels_numeric):
    ...
    df_maths_year['ID'] = df_maths_year['ID'].astype(str)

    # Subsetting
    class_CG = student_CG[student_CG['Student ID'].isin(df_maths_year['ID'])] # Class
    year_CG = student_CG[student_CG['Year Level'] == year_level] # Year-level
    # Campus
    campus = df_maths_year['ClassCampus'].iloc[0]
    campus_CG = student_CG[student_CG['Year Level'] == campus]

    # Call plots
    # Bar Plot
    plot_CG(year_level_df= year_CG, class_df= class_CG,
...
for year_level in sorted(year_levels_numeric):
    ...
    # Check for eanough data before clustering
    if not has_maths or not has_reading:
        display(Markdown("*Note: Clustering analysis requires both PAT Maths and Reading data. Skipping for this class.*\n"))
        continue
    ...
    # Converting 'ID' column
    df_maths_year['ID'] = df_maths_year['ID'].astype(str)
    # Get class_CG again as the previous iteration was a locally created variable (i.e. only exists in the first for loop)
    class_CG = student_CG[student_CG['Student ID'].isin(df_maths_year['ID'])] # Class

    # Adding prefix to non-key columns
    df_maths_prefixed = df_maths_year.rename(
        columns = lambda c: f"Maths_{c}" if c not in keys else c
    )
    df_reading_prefixed = df_reading_year.rename(
    ...
    # Adding WAM/GPA information
    clustering_df['WAM or GPA'] = clustering_df['ID'].map(
        class_CG.set_index('Student ID')['WAMorGPA']
    )

    # Making dataframe for clustering analysis
    # Making copy for analysis (excluding students with missing data)
    ...
    # Drop rows with any NaN values from analysis
    X = X.dropna()
...
# For loop
for year_level in sorted(year_levels_numeric): #sorted(year_levels_in_class):
    display(Markdown(f"## Year {year_level} Results"))
    ...
    # Make table subsets for printing
    # Scale Scores
    scale_table = clustering_df[["Student ID","Student Name", "Year Level", "PAT Maths Scale Score", "PAT Reading Scale Score", "Student Achievement"]]
    styled_scaletbl = scale_table.style.set_table_styles([
        {'selector': 'th', 'props': [('text-align', 'center')]},
    ]).hide(axis = "index")
    # Percentile Scores
    percentile_table = clustering_df[["Student ID","Student Name", "Year Level", "PAT Maths Percentile", "PAT Reading Percentile", "Student Achievement"]]
    styled_percentiletbl = percentile_table.style.set_table_styles([
        {'selector': 'th', 'props': [('text-align', 'center')]},
    ]).hide(axis="index")

    # Display scale table
    display(Markdown(f"### Year {year_level} - Scale Scores"))
    display(styled_scaletbl)

    # Page break
    display(Markdown("{{< pagebreak >}}"))

    # Display percentile table
    display(Markdown(f"### Year {year_level} - Percentile Scores"))
    display(styled_percentiletbl)
```