...
# Get year levels in the class
year_levels_in_class = df_maths['StudentYearLevel'].unique()
```
...
# Alternatively:
# stud_year_level = df_maths.groupby('StudentYearLevel').size().to_dict()
...
for year_level in sorted(year_levels_in_class):
    print(f"Year {year_level} Results")

    # Filter data
    df_maths_year = df_maths[df_maths['StudentYearLevel'] == year_level]
    df_reading_year = df_reading[df_reading['StudentYearLevel'] == year_level]

    # Get year-specific data
    year_data = substrand_norm_df[f'Year{year_level}']

    # Call relevant q_strands_match dataframe
    q_strands_maths = year_level_q_strands_maths[f'Year{year_level}']
    q_strands_reading = year_level_q_strands_reading[f'Year{year_level}']

    # Call plots
    # Maths
    compare_plots(dataframe = year_data['substrands_maths'], class_df = df_maths_year,
                  strand_col_name = 'Strand', substrand_col_name = 'Sub-strand',
                  test = 'PAT Maths', plot_title = None)
    # Reading
    compare_plots(dataframe = q_strands_reading, class_df = df_reading_year,
                  strand_col_name = 'Strand', substrand_col_name = 'Sub-strand',
...
for year_level in sorted(year_levels_in_class):
    ...
    year_maths = year_level_dataframes_maths[f'Year{year_level}'] # contains the students_math_match_yX
    year_reading = year_level_dataframes_reading[f'Year{year_level}']
    # year_maths = students_maths_match[students_maths_match['StudentYearLevel'] == year_level]
    # year_reading = students_reading_match[students_reading_match['StudentYearLevel'] == year_level]

    # Call plots
    # Maths
    students_vs_year_score(df1 = year_maths, df2 = df_maths_year,
    ...
    # Reading
    students_vs_year_score(df1 = year_reading, df2 = df_reading_year,
                            yearlevel_scores = 'Scale', student_scores = 'Scale',
                            student_names = 'StudentNameExternal',
                            title = 'PAT Maths - Student Scores vs School-wide Year-Level Scores') 
```
...
for year_level in sorted(year_levels_in_class):
    ...
    campuses = df_maths_year['Campus'].unique()

    # Get year-specific data
    year_data = substrand_norm_df[f'Year{year_level}']

    # Get year-level comparison data
    students_maths_match = year_level_dataframes_maths[f'Year{year_level}']
    students_reading_match = year_level_dataframes_reading[f'Year{year_level}']
    # Get campus set
    ...
    # Call plots
    # Maths
    students_vs_year_score(df1 = campus_set_maths, df2 = df_maths_year,
                            yearlevel_scores = 'Scale', student_scores = 'Scale',
                            student_names = 'StudentNameExternal',
                            title = 'PAT Maths - Student Scores vs Campus-wide Year-Level Scores')
    # Reading
    students_vs_year_score(df1 = campus_set_maths, df2 = df_reading_year,
                            yearlevel_scores = 'Scale', student_scores = 'Scale',
                            student_names = 'StudentNameExternal',
...
for year_level in sorted(year_levels_in_class):
    ...
    # Call plots
    # Maths
    score_bellcurve(dataframe = df_maths_year, mean = year_data['mean_maths'],
                    std=year_data['std_maths'], student_scale_col='Scale',
                    title = "PAT Maths - Student Scale Scores Relative to Australian Norms")
    # Reading
    score_bellcurve(dataframe = df_reading_year, mean = year_data['mean_reading'],
...
for year_level in sorted(year_levels_in_class):
    ...
    year_CG = student_CG[student_CG['Year Level'].isin(year_level)] # Year-level
    # Campus
...
for year_level in sorted(year_levels_in_class):
    ...
    # Subsetting
    clustering_df = clusterin_df[['ID', 'StudentNameExternal', 'Maths_StudentYearLevel', 'Maths_Scale', 'Reading_Scale', 
                                #'Spelling_Scale',
    ...
    clustering_df['WAM or GPA'] = clustering_df['ID'].map(
        class_CG.set_index('Student ID')['WAM or GPA']
    )
